...
def create_app():
    ...
    # Middleware to log incoming request headers for MCP routes and store in context
    @app.middleware("http")
    async def log_mcp_headers(request, call_next):
        if request.url.path.startswith("/mcp"):
            logger.debug(f"@@@@ Incoming request to {request.url.path}")
            logger.debug(f"@@@@ All incoming headers: {dict(request.headers)}")
            # Store headers in context variable for access in MCP proxy
