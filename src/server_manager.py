...
class ServerManager:
    ...
    async def _create_or_replace_container_impl(
        ...
        # For podman, use :U to automatically chown the volume to the container user
        volume_options = ":rw,U" if settings.CONTAINER_ENGINE == "podman" else ":rw"
        ...
        config: dict[str, Any] = {
            ...
            "HostConfig": {
                "Binds": [f"{src_data_dir}:{dst_data_dir}{volume_options}"],
